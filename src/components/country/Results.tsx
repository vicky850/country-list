  ...
  useEffect(() => {
    const getCountries = async (url: string) => {
      try {
        ...
        let fetchedCapital = false;
        if (!response.ok) {
          //throw new Error("Failed to fetch data");
          if (!fetchedCapital) {
            fetchedCapital = true;
            getCountries(`https://restcountries.com/v3.1/capital/${debouncedSearchTerm}`);
          } else {
            throw new Error("Failed to fetch data");
          }
        }
        ...
        <div className="flex w-full flex-col items-center">
          <div className="w-full max-w-[90%] justify-between space-y-8 pb-[2.2rem] pt-[2.2rem] sm:flex sm:space-y-0 lg:max-w-[100%]">
            ...
            <div className="mb-3 cursor-pointer sm:ml-6">
              ...
              >
                {order === "" ? "Sort^" : order==='asc' ? <LiaSortNumericDownSolid /> : <LiaSortNumericUpAltSolid />}
              </Button>
